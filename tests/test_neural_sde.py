...
def test_sde_with_d_protocol_zero_behaves_identically():
    ...
    ctx = _make_context(birth_death())
    sde = CRNNeuralSDE(SDEConfig(d_model=16, d_hidden=32, n_noise_channels=4, d_protocol=0), n_species=1)
...
def test_sde_with_d_protocol_positive_accepts_embedding():
    ...
    sde = CRNNeuralSDE(SDEConfig(d_model=16, d_hidden=32, n_noise_channels=4, d_protocol=d_protocol), n_species=1)
    state = torch.tensor([5.0])
...
def test_solver_with_input_protocol_clamps_external_species():
    ...
    ).BipartiteGNNEncoder(__import__(
    ...
    sde = CRNNeuralSDE(SDEConfig(d_model=16, d_hidden=32, n_noise_channels=4), n_species=2)
    solver = EulerMaruyamaSolver(SDEConfig(d_model=16, clip_state=True))
    t_span = torch.linspace(0.0, 10.0, 50)
    traj = solver.solve(
        sde, torch.tensor([0.0, 0.0]), ctx, t_span, dt=0.05,
...
def test_solver_raises_without_external_mask_when_protocol_provided():
    ...
    from crn_surrogate.crn import EMPTY_PROTOCOL, single_pulse, InputProtocol
    from crn_surrogate.crn.inputs import PulseEvent, PulseSchedule
    ...
    with pytest.raises(ValueError, match="external_species_mask"):
        solver.solve(sde, torch.tensor([0.0]), ctx, torch.linspace(0, 5, 10), dt=0.1,
                     input_protocol=protocol)
