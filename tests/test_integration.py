...
- CRN definition → tensor repr → encoder → SDE forward pass → trajectory shape.
...
def test_end_to_end_birth_death_trajectory_shape_and_metadata():
    """Full forward pass for birth-death: verify trajectory shape and attributes."""
    config = ModelConfig(
...
def test_trainer_completes_and_returns_valid_training_result(tmp_path):
    """Trainer.train() must complete without error and return finite train losses."""
